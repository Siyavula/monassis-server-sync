    ...
    with open('export_initial_sync_from_server.pickle', 'rb') as fp:
        while True:
            try:
                ...
                break
            record_database.insert_record(config, struct['section_name'], struct['record_id'], struct['record'])
            record_database.insert_hash(config, struct['section_name'], struct['record_id'], struct['hash'])
