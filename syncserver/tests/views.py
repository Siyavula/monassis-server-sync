...
def get_hash_actions_view(request):
    ...
    GET /{name}/hash-actions
        < {'lock_key': string, 'sync_time': iso8601,
           'client_vars': user-defined-config-vars (optional)}
        > {'hash_actions': {section_name (string): [
            [user-defined-record-id, ['insert', user-defined-hash] or
